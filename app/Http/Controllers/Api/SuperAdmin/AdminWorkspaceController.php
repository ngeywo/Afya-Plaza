...
{
    /**
     * The "view workspace" feature. The Super Admin may switch their read-only
     * workspace context (platform-level, a specific doctor, or a facility)
     * without impersonating anyone. State is cache-scoped to the admin.
     */
    public function workspace(Request $request): JsonResponse
    {
        $context = Cache::get('admin.workspace.'.$request->user()->id);

        if ($context && $context['type'] === 'doctor') {
            $doctor = Doctor::find($context['id']);
            if ($doctor) {
                return $this->doctorDashboard($request, $doctor);
            }
        }
        if ($context && $context['type'] === 'facility') {
            $facility = Facility::find($context['id']);
            if ($facility) {
                return $this->facilityDashboard($request, $facility);
            }
        }

        return $this->platformSummary($request);
    }

    public function switchWorkspace(Request $request): JsonResponse
    {
        $request->validate([
            'type' => 'required|in:platform,doctor,facility',
            'id' => 'required_unless:type,platform|integer',
        ]);

        $context = ['type' => $request->input('type'), 'id' => $request->input('id')];

        if ($context['type'] === 'doctor') {
            $doctor = Doctor::findOrFail($context['id']);
            Cache::put('admin.workspace.'.$request->user()->id, $context, now()->addDays(7));

            return $this->doctorDashboard($request, $doctor);
        }

        if ($context['type'] === 'facility') {
            $facility = Facility::findOrFail($context['id']);
            Cache::put('admin.workspace.'.$request->user()->id, $context, now()->addDays(7));

            return $this->facilityDashboard($request, $facility);
        }

        Cache::forget('admin.workspace.'.$request->user()->id);

        return $this->platformSummary($request);
    }

    public function exitWorkspace(Request $request): JsonResponse
    {
        Cache::forget('admin.workspace.'.$request->user()->id);
    ...
    private function platformSummary(Request $request): JsonResponse
    {
        $today = Carbon::today();

        return response()->json(['data' => [
            'workspace_type' => 'platform',
            'context' => Cache::get('admin.workspace.'.$request->user()->id) ?? ['type' => 'platform', 'id' => null],
            'counts' => [
                'doctors' => Doctor::count(),
                'facilities' => Facility::count(),
                'appointments_today' => Appointment::whereDate('appointment_date', $today)->whereIn('status', ['pending', 'confirmed'])->count(),
                'clinics_today' => ClinicSession::whereDate('session_date', $today)->whereIn('status', ['pending', 'confirmed'])->count(),
            ],
        ]]);
    }
